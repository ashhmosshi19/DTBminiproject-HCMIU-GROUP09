...
* **Search Engine (`result.php`):** An advanced, order-independent search algorithm. It tokenizes user input and searches across multiple table columns simultaneously, ensuring highly accurate results regardless of the keyword order.
* **Status Updates (`update.php`):** Allows administrators to quickly update a bus's operational status (Active, Inactive, Maintenance) and log its latest maintenance date.
